...
\begin{algorithm}[H]
    ...
    \KwOut{$Y$ - The resulting vector of values outputted by the neural network}
...
% $$\frac{\delta L}{\delta w^0_{0,i}} = \frac{2(\sigma(W_0 \cdot \sigma(W_1 \cdot \vec{y^2})) - \hat{y})((\sigma(W_0 \cdot \sigma(W_1 \cdot \vec{y^2}))) - (\sigma(W_0 \cdot \sigma(W_1 \cdot \vec{y^2})))^2)}{1 + e^{-(\sum_h^{}{(w^1_{i,h} \cdot y^2_{h})})}}$$
...
% WAIT WOULD WE HAVE THE FIRST SECTION WITH THE 2 AND SUCH CAUSE ISNT' THAT IN THE FIRST PART?
% no wait it wouldn't be because dx of (2f(x))^2 = 2(2f(x)) * (2f'(x)) * 1(x^0)
...
\begin{algorithm}[H]
    ...
    \KwIn{$Y$ - The input vector of the matrix, size $h$ x $1$
    \newline$W^1$ - The weight matrix of the hidden layer, of dimensions $m$ x $h$. These are the weights of each of the inputs as they pass from the inputs to the hidden layer.
    \newline$W^0$ - The weight matrix of the output layer, of dimensions $1$ x $m$. These are the weights applied to the values as they pass from the hidden layer to the output layer (which is size 1)
    \newline$S$ - The step factor. This is the multiple that the "gradient" will be multiplied by in order to improve the network
    \newline$\hat{Y}$ - The expected output of the neural network}
    \KwOut{$W^1$ - The updated weight matrix of the hidden layer
    \newline$W^0$ - The updated weight matrix of the output layer}
